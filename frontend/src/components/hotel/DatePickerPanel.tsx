...
const DatePickerPanel = ({
                    ...
                    {cells.map((isoDate, index) => {
                        ...
                        return (
                            <button
                                ...
                                className={`mx-auto h-11 w-11 rounded-full text-lg font-medium transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                                    isSelected
    ...
    return (
        <div
            className={`absolute top-[calc(100%+12px)] z-50 w-[min(940px,calc(100vw-2rem))] transform-gpu rounded-[32px] border border-gray-200 bg-[#f5f5f5] p-6 shadow-2xl transition-[transform,opacity] duration-[980ms] ease-[cubic-bezier(0.16,1,0.3,1)] will-change-transform ${placementClass} ${
                isOpen ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-2 opacity-0"
            }`}
        >
            <div className="mb-8 flex justify-center">
                <div className="flex w-96 rounded-full bg-gray-300 p-1">
                    <button
                        type="button"
                        onClick={() => setMode("exact")}
                        className={`w-1/2 rounded-full py-2 text-base font-semibold transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                            mode === "exact" ? "bg-white text-gray-900" : "text-gray-700"
                        }`}
                    >
                        Ngày
                    </button>
                    <button
                        ...
                        className={`w-1/2 rounded-full py-2 text-base font-semibold transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                            mode === "flex" ? "bg-white text-gray-900" : "text-gray-700"
                        }`}
                    >
                        Linh hoạt
                    </button>
                            ...
                            {exactnessOptions.map((option) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`rounded-full border px-5 py-2 text-sm font-medium transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                                            isActive
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                            {flexibleDurationOptions.map((option) => (
                                <button
                                    ...
                                    className={`rounded-full border px-7 py-2.5 text-[18px] font-medium leading-none transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] md:text-[20px] ${
                                        flexDuration === option
                                            ? "border-gray-900 bg-white text-gray-900"
                                            : "border-gray-300 bg-transparent text-gray-700 hover:border-gray-400"
                                    }`}
                            ...
                            {flexibleMonths.map((monthStart) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`min-w-[190px] rounded-[26px] border px-6 py-9 text-center transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                                            isActive
