        ...
        <div className="relative min-h-screen overflow-hidden bg-[#eef2f7] font-sans">
            ...
            <div className="pointer-events-none absolute inset-0 hidden xl:block">
                {photoCards.map((card) => (
                    <div
                        key={card.title}
                        className={`absolute w-[270px] rounded-[30px] border border-white/70 bg-white/78 p-3 shadow-[0_32px_80px_-40px_rgba(15,23,42,0.55)] backdrop-blur-lg ${card.className}`}
                    >
                        <img src={card.image} alt={card.alt} className="h-72 w-full rounded-[24px] object-cover" />
